...
public class SketchyEffectURP : ScriptableRendererFeature {
    ...
    void initAll(int width, int height)
    {
        ...
        shaders["Image"] = createShaderOptSuff("flockaroo_Sketchy/ImageHDRP",defineModeSuffixes);
        ...
        shaders["Geom_A"] = createShaderOptSuff("flockaroo_Sketchy/Geom_AHDRP",defineModeSuffixes);
    ...
    public void Render(CommandBuffer cmd, RenderTexture src, RenderTexture dest) {
        ...
        {
            Debug.Log("Effect 1st init (or Resolution changed)");
        ...
        foreach( string buffName in bufferOrder )
        {
            ...
            mat.SetFloat("Colors",s.Colors);
            mat.SetFloat("ColorsBlur",s.ColorsBlur);
            mat.SetFloat("MasterFade",s.MasterFade);
            mat.SetFloat("Noise",s.Noise);
            mat.SetFloat("Outlines",s.Outlines);
            mat.SetFloat("OutlinesBlur",s.OutlinesBlur);
            mat.SetFloat("StrokeSpread",s.StrokeSpread);
            mat.SetFloat("StrokeThresh",s.StrokeThresh);
            mat.SetFloat("VignWidth",s.VignWidth);
            mat.SetFloat("Vignetting",s.Vignetting);
            mat.SetFloat("linear2Gamma", s.linear2Gamma?1.0f:0.0f);
            mat.SetFloat("Geom_A_NumTriangles",s.NumTriangles);
            meshNumTri["Geom_A"]=(int)s.NumTriangles;
            //###MatParams
    ...
    public override void Create()
    {
        if(m_ScriptablePass == null)
            m_ScriptablePass = new EffectRenderPass("FlockarooSketchy",ref settings,settings.WhenToApply);
