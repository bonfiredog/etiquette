...
public class AquarelleEffectURP : ScriptableRendererFeature {
    ...
    public class EffectSettings
    {
        ...
        [Range(0.0f, 1.0f)]
        public float UseMask = 0.000000f;
        //###PublicVars
        [Header("Other")]
    ...
    Texture getTexture(string name)
    {
        ...
        if(name=="effectMask")         return s.maskTexture;
        return mainTex;
    ...
    public void Render(CommandBuffer cmd, RenderTexture src, RenderTexture dest) {
        ...
        meshNumTri["Geom_A"]=((int)(s.PredrawAmount*(float)(0x10000)))/2*2;
        ...
        foreach( string buffName in bufferOrder )
        {
            ...
            mat.SetFloat("ContentVignAspect",s.ContentVignAspect);
            mat.SetFloat("ContentVignSharp",s.ContentVignSharp);
            mat.SetFloat("ContentVignSize",s.ContentVignSize);
            ...
            mat.SetFloat("PredrawStrength",s.PredrawStrength);
            mat.SetFloat("Spread",s.Spread);
            mat.SetFloat("SrcBrightness",s.SrcBrightness);
            mat.SetFloat("SrcColor",s.SrcColor);
            mat.SetFloat("SrcContrast",s.SrcContrast);
            mat.SetFloat("PreGrad",s.fastGrad?1.0f:0.0f);
            mat.SetFloat("ColorSpread",s.ColorSpread);
            mat.SetFloat("StrokeScale",s.StrokeScale);
            mat.SetFloat("RimDry",s.RimDry);
            mat.SetFloat("Vignette",s.Vignette);
            mat.SetFloat("UseMask",s.UseMask);
    ...
    public override void Create()
    {
        if(m_ScriptablePass==null)
            m_ScriptablePass = new EffectRenderPass("FlockarooAquarelle",ref settings,settings.WhenToApply);
